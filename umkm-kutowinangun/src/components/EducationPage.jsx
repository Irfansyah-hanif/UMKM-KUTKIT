...
import { GraduationCap, ZoomIn, ZoomOut, X, Download } from 'lucide-react';
...
export default function EducationPage() {
  ...
  return (
    <main className="w-full px-4 sm:px-8 lg:px-12 py-8 flex-1 space-y-8 relative z-10">
      {/* Header Halaman Edukasi */}
      <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-xs border border-sky-100/80 space-y-2">
        ...
        <p className="text-xs sm:text-sm text-sky-700 font-medium">
          Materi infografis dan modul edukasi untuk meningkatkan kapasitas serta higienitas pelaku usaha mikro lokal.
        </p>
      </div>
        ...
        {edukasiList.map((item) => (
          ...
          >
            ...
            <div
              ...
              className="relative h-80 sm:h-96 bg-sky-50 overflow-hidden cursor-pointer"
            >
              <img
                src={item.gambar}
                alt={item.judul}
            ...
            <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
              <div className="space-y-2">
                <h3 className="font-black text-sky-950 text-base group-hover:text-sky-600 transition-colors">
                  {item.judul}
                </h3>
      ...
      {/* LIGHTBOX MODAL - DENGAN FITUR SCROLL FULL PAN SAAT ZOOM */}
      {previewImage && (
        <div
          onClick={handleClosePreview}
          className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 cursor-pointer transition-opacity"
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="relative max-w-5xl w-full bg-slate-900 text-white rounded-3xl overflow-hidden shadow-2xl border border-white/10 flex flex-col h-[90vh]"
          >
            ...
            <div className="p-3 sm:p-4 bg-slate-900 border-b border-slate-800 flex flex-wrap items-center justify-between gap-3 shrink-0">
              <h4 className="text-xs sm:text-sm font-bold text-white truncate max-w-[200px] sm:max-w-md">
                {previewImage.judul}
              </h4>

              {/* Kontrol Tombol Zoom */}
              <div className="flex items-center gap-1.5 bg-slate-800 px-2 py-1 rounded-2xl border border-slate-700">
                <button
                  onClick={handleZoomOut}
                  disabled={zoomScale <= 0.8}
                  className="p-1.5 hover:bg-slate-700 text-slate-200 rounded-xl transition-colors disabled:opacity-40 cursor-pointer"
                  title="Zoom Out (-)"
                >
                  <ZoomOut className="w-4 h-4" />
                </button>

                <span className="text-[11px] font-bold text-sky-400 px-2 min-w-[45px] text-center select-none">
                ...
                <button
                  ...
                  className="px-2.5 py-1 hover:bg-slate-700 text-slate-200 hover:text-white text-[11px] font-extrabold rounded-xl transition-colors cursor-pointer ml-1"
              ...
              <button
                onClick={handleClosePreview}
                className="w-8 h-8 rounded-full bg-white/10 text-white hover:bg-rose-500 flex items-center justify-center cursor-pointer transition-all shrink-0"
              >
                <X className="w-4 h-4" />
              </button>
            </div>

            {/* Container Scroll Gambar yang Luas */}
            <div className="flex-1 overflow-auto p-4 bg-black/60 scrollbar-thin flex">
              <div className="m-auto flex items-center justify-center min-w-full min-h-full p-4">
                <div
                ...
                >
                  <img
                    ...
                    className="max-w-none max-h-[75vh] w-auto object-contain rounded-lg shadow-2xl block"
                  />
                </div>
              </div>
            </div>
